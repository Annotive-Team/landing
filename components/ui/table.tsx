...
>(({ className, ...props }, ref) => (
  <div className="table-container relative w-full overflow-auto" id="table-container">
    <table
      ref={ref}
      className={cn("main-table w-full caption-bottom text-sm", className)}
      {...props}
    />
  </div>
))
...
>(({ className, ...props }, ref) => (
  <thead ref={ref} className={cn("table-header [&_tr]:border-b", className)} {...props} />
))
...
>(({ className, ...props }, ref) => (
  <th
    ref={ref}
    className={cn(
      "table-head h-12 px-4 text-left align-middle font-medium text-muted-foreground [&:has([role=checkbox])]:pr-0",
      className
    )}
    {...props}
  />
